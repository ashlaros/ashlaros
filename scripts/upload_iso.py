...
def main() -> int:
    ...
    for path, content_type, _ in found:
        ...
        # Stored on the object, not added by the worker: a download now
        # redirects to the bucket's own hostname, which serves whatever
        # metadata the object carries and knows nothing about our handlers.
        # Without it a browser saves the image under the last path segment
        # or its own guess rather than the name the checksum refers to.
