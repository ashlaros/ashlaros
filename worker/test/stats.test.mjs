...
test('a resumed download does not count as many downloads', () => {
  ...
  assert.equal(counts('2026.09.11/ashlaros.iso', 206, 'GET'), false);
  assert.equal(counts('2026.09.11/ashlaros.iso', 200, 'GET'), true);
});

test('a revalidation transfers nothing and does not count', () => {
  assert.equal(counts('2026.09.11/ashlaros.iso', 304, 'GET'), false);
});
...
test('serving an iso counts it, serving a package does not', async () => {
  ...
  const pkgEnv = {
    PACKAGES: bucketOf(['x86_64/ashlaros.db.tar.gz']),
    ANALYTICS_ENGINE: analytics(),
  };
  await worker.fetch(get('packages.ashlaros.download', 'x86_64/ashlaros.db.tar.gz'), pkgEnv, {});
...
test('an unset token renders the page rather than a 500', async () => {
  ...
  const response = await worker.fetch(get('iso.ashlaros.download', 'stats'), env, {});
  assert.equal(response.status, 200);
...
test('an archived month is published even with no token', async () => {
  ...
  const response = await worker.fetch(get('iso.ashlaros.download', 'stats.json'), env, {});
...
test('archiving writes one key holding the month total', async () => {
  ...
  assert.match(sql, /FROM "ashlaros-iso-downloads"/);
