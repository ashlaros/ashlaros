...
export function validateEvents(events) {
  ...
  let perTick = 0;
  for (const event of events) {
    // exactly two, not at-least-two: the other game's [action, tick,
    // value] triple would otherwise be scored under these rules, which is
    // precisely the confusion one verify() over two simulations invites
    if (!Array.isArray(event) || event.length !== 2) return 'each event is [action, tick]';
    const [action, tick] = event;
    if (!Number.isInteger(action) || !Number.isInteger(tick)) {
      return 'action and tick must be integers';
    }
    // The Rust core ignores an action it does not recognise, which would
    // score a log naming one rather than refusing it. Refused here, where
    // every submission passes, instead of trusted to the simulation.
    if (action < 0 || action > ACTIONS.HOLD) return 'unknown action';
